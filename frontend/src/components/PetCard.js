    ...
    return (
        ...
        <div style={{ "maxWidth": "1000px", margin: "auto"}}>
            ...
            <Grid columns={2} stretched>
                <Grid.Column width={5}>
                    <Segment>
                        <h4>{props.pet.gender === "M" ? <Icon circular name="mars"/> : <Icon circular name="venus"/>} {`${props.pet.name} (${props.pet.date_of_birth})`}</h4>
                        <img src={corgiPuppy} className="ui large image" alt="pet" />
                        <h4 onClick={()=> { showMakeApptForm("mini"); dispatch({ type: 'CURRENT_PET', payload: props.pet})}}><Icon circular name="calendar plus outline"/>{`Make an appointment for ${props.pet.name}`}</h4>
                        <div>
                            {props.pet.appointments.map( appointment => 
                                <Segment style={{display: "flex", justifyContent: "space-between", alignItems: "center"}}>
                                    ...
                                    {appointment.user_confirmed === true && appointment.employee_confirmed === true ?
                                        ...
                                        :
                                        <button className="ui icon button" onClick={()=> userConfirmed(appointment)}>
                                            <i className="check icon"></i>
                                        </button>
                                    }
                                </Segment>
